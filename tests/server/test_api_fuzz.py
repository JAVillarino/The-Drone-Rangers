...
from server import jobs_api, scenarios_api, main
import threading
...
def app_fuzz():
    ...
    with open(jobs_db, "wb") as f: pickle.dump([], f)
    jobs_api._REPO = None
    ...
    with open(scen_db, "wb") as f: pickle.dump([], f)
...
def test_fuzz_invalid_json(app_fuzz):
    ...
    res = client.post("/api/jobs", data="{invalid_json:", content_type="application/json")
